...
class Env:
    ...
    def get_next_episode(self, episode_id, scene_id):
        ...
        if self._episode_source == 'dataset':
            if scene_name not in self._episode_datasets.keys():
                with open('episodes/{}_{}.txt'.format(scene_name, self.difficulty),'r') as f:
                    episodes = f.readlines()
                processed_episodes = []
                for ep_str in episodes:
                    id, spx, spy, spz, srx, sry, srw, srz, tx, ty, tz = ep_str.split(' ')
                    tz = tz[:-2]
                    episode = {}
                    episode['id'] = id
                    episode['start_position'] = [float(spx),float(spy),float(spz)]
                    episode['start_rotation'] = q.from_float_array([float(srx),float(sry),float(srw), float(srz)])
                    episode['goal_position'] = [float(tx),float(ty),float(tz)]
                    processed_episodes.append(episode)
                self._episode_datasets.update({scene_name: processed_episodes})
                self._current_scene_episode_idx = 0
            ...
            self.start_rotation = self._sim.get_agent_state(0).rotation
            self._sim.set_agent_state(self.start_position, self.start_rotation)
            start_position, start_rotation = self.start_position, self.start_rotation
            num_try = 0
            while True:
                if self.MIN_DIST < 5.0:
                    random_dist = self.MIN_DIST + np.random.rand() * (self.MAX_DIST - self.MIN_DIST)
                    ...
                    if abs(goal_position[1] - start_position[1]) > 0.5: continue

                geodesic_dist = self._sim.geodesic_distance(start_position, goal_position)
                valid_dist = (geodesic_dist < self.MAX_DIST) * (geodesic_dist > self.MIN_DIST)
                if self._sim.is_navigable(goal_position) and valid_dist:
                    goal = NavigationGoal(**{'position': goal_position})
                    goals.append(goal)
                    start_position = goal_position

                if len(goals) >= self._num_goals:
                    break
                num_try += 1
                if num_try > 1000 and len(goals) >= 1 : break
                elif num_try > 100 and len(goals) == 0:
                    self.start_position = self._sim.sample_navigable_point()
                    self.start_rotation = self._sim.get_agent_state(0).rotation
                    self._sim.set_agent_state(self.start_position, self.start_rotation)
                    start_position, start_rotation = self.start_position, self.start_rotation
