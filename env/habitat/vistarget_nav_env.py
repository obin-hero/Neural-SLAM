...
from env_utils.graph_map import TopDownGraphMap
...
class VisTargetNavEnv(RLEnv):
    ...
    def __init__(self, config: Config, dataset: Optional[Dataset] = None):
        ...
        if 'GMT' in config.POLICY and RENDER:
            ...
            task_config.TASK.TOP_DOWN_GRAPH_MAP.DRAW_SOURCE = True
            task_config.TASK.TOP_DOWN_GRAPH_MAP.DRAW_SHORTEST_PATH = True
            task_config.TASK.TOP_DOWN_GRAPH_MAP.FOG_OF_WAR.VISIBILITY_DIST = 2.0
            task_config.TASK.TOP_DOWN_GRAPH_MAP.FOG_OF_WAR.FOV = 360
            task_config.TASK.TOP_DOWN_GRAPH_MAP.FOG_OF_WAR.DRAW = True
            task_config.TASK.TOP_DOWN_GRAPH_MAP.DRAW_VIEW_POINTS = False
            task_config.TASK.TOP_DOWN_GRAPH_MAP.DRAW_GOAL_POSITIONS = True
            task_config.TASK.TOP_DOWN_GRAPH_MAP.DRAW_GOAL_AABBS = False
            task_config.TASK.TOP_DOWN_GRAPH_MAP.NUM_TOPDOWN_MAP_SAMPLE_POINTS = 40000
    ...
    def build_path_follower(self):
        ...
        self.follower = ShortestPathFollower(self._env.sim, 0.25, False)
    ...
    def get_best_action(self, goal=None):
        ...
        if goal is None and (act is None or act is 0) and self.curr_goal_idx < self.num_goals -1:
            self.curr_goal_idx +=1
            self.curr_goal = self.current_episode.goals[self.curr_goal_idx]
            act = self.follower.get_next_action(self.curr_goal.position)
        #print(self.curr_goal, self.current_episode.goals, self.current_position)
        return act
...
#'state': self.habitat_env.sim.get_agent_state()}
    ...
    def step(self, action):
        ...
        #if not done and self._part_success():
        #    self.curr_goal_idx += 1
        #    if self.num_goals > self.curr_goal_idx:
        #        self.curr_goal = self.current_episode.goals[self.curr_goal_idx]
